...
describe("Katana Morpho Tests", async function () {
  ...
  const katanaForkConfig = {
    ...
    forking: {
      url: process.env.KATANA_RPC_URL || "https://rpc-katana.t.conduit.xyz/MekJWT3Kd9YJyktBPJxVMk75TaFG7pdvq",
    },
  };
  ...
  it("Should deposit USDC into Morpho on Katana", async function () {
    ...
    const usdc = await viem.getContractAt("interface/IERC20.sol:IERC20", USDC_ADDRESS);
    ...
    if (!morphoCode || morphoCode === "0x") {
      ...
      console.log("This test demonstrates the Morpho deposit interface pattern");
    ...
    try {
      ...
      console.log(`- Amount deposited: ${formatUnits(depositAmount, usdcDecimals)} USDC`);
  ...
  it("Should query Morpho market information", async function () {
    ...
    const contractCode = await publicClient.getCode({ address: MORPHO_ADDRESS });
