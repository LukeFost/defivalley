...
contract GameController is Ownable, ReentrancyGuard {
    ...
    function plantSeed(
        ...
        bytes32 txId = keccak256(
            abi.encodePacked(msg.sender, seedId, block.timestamp, amount)
        );
    ...
    function _initiateCrossChainCommand(
        ...
        if (command == Command.DEPOSIT) {
            ...
            emit CrossChainHarvestInitiated(
                msg.sender,
                amount,
                txId,
                DESTINATION_CHAIN
    ...
    function harvestSeed(
        ...
        uint256 estimatedYield = _calculateEstimatedYield(
            seed.amount,
            config.growthTime
        );
    ...
    function batchHarvestSeeds(
        ...
        for (uint256 i = 0; i < seedIds.length; i++) {
            ...
            totalEstimatedYield += _calculateEstimatedYield(
                seed.amount,
                config.growthTime
            );
        }
        ...
        bytes32 batchHarvestTxId = keccak256(
            abi.encodePacked(
                msg.sender,
                seedIds,
                "batch_harvest",
        ...
        for (uint256 i = 0; i < seedIds.length; i++) {
            ...
            emit SeedHarvested(
                msg.sender,
                seedIds[i],
                _calculateEstimatedYield(
                    players[msg.sender].seeds[seedIds[i]].amount,
                    seedTypes[players[msg.sender].seeds[seedIds[i]].seedType]
                        .growthTime
                )
            );
        }
     ...
     */
    function _calculateEstimatedYield(
        uint256 amount,
        uint256 growthTime
    ) internal pure returns (uint256) {
    ...
    function getSeedInfo(
        address player,
        uint256 seedId
    ) external view returns (SeedPosition memory seed) {
     ...
     */
    function configureSeedType(
        uint256 seedType,
        SeedConfig calldata config
    ) external onlyOwner {
