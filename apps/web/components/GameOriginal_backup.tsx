...
class MainScene extends Phaser.Scene {
  ...
  init(data: { 
    ...
    console.log('✅ MainScene.init() completed successfully');
  }

  preload() {
    console.log('🎮 MainScene.preload() started');
    console.log('📦 Phaser game instance:', this.game);
    console.log('📦 Scene manager:', this.scene);
  ...
  async create() {
    ...
    console.log('🎨 Game dimensions:', this.game.config.width, 'x', this.game.config.height);
    console.log('🎨 Renderer type:', this.game.renderer.type === Phaser.WEBGL ? 'WebGL' : 'Canvas');
    
    // Log all loaded textures for debugging
    const textureManager = this.textures;
    console.log('🖼️ Loaded textures:', textureManager.list);
    
    // Check if main tileset loaded
    if (!textureManager.exists('cliffs_grass_tileset')) {
      console.error('❌ CRITICAL: Main tileset not loaded!');
    } else {
      console.log('✅ Main tileset loaded successfully');
    }
    ...
    console.log('🎮 Scene is now ready and should be visible');
  }
    ...
    buildingConfigs.forEach(config => {
      ...
      hotspot.on('pointerdown', (pointer: Phaser.Input.Pointer) => {
        if (pointer.rightButtonDown()) {
          console.log(`🏗️ Right-clicked on ${config.name} - Context menu should appear`);
          ...
          if (config.callback) {
            console.log(`🏗️ ${config.name} clicked - Entering directly!`);
            config.callback();
          } else {
      ...
      hotspot.on('pointerover', () => {
        ...
        console.log(`👆 Hovering over ${config.name} - Click to enter!`);
      });
  ...
  setupDevTools() {
    if (typeof window !== 'undefined') {
      ...
      (window as any).testBuildingVisibility = () => {
        ...
        console.log('Created CORRAL PLACEHOLDER test at (600, 300)');
      ...
      (window as any).listAllScenes = () => {
        ...
        console.log('  - MainScene (Farm)', this.scene.isActive('MainScene') ? '✅ Active' : '⏸️ Inactive');
        console.log('  - CorralScene (Trader)', this.scene.isActive('CorralScene') ? '✅ Active' : '⏸️ Inactive');
        console.log('  - OrchardScene (Gardener)', this.scene.isActive('OrchardScene') ? '✅ Active' : '⏸️ Inactive');
        console.log('  - WellScene (Mystic)', this.scene.isActive('WellScene') ? '✅ Active' : '⏸️ Inactive');
      };
      ...
      console.log('  🎬 VISUAL NOVEL SCENE TESTING:');
      console.log('  - testCorralScene(): Test transition to Corral Scene (Trader)');
      console.log('  - testOrchardScene(): Test transition to Orchard Scene (Gardener)');
      console.log('  - testWellScene(): Test transition to Well Scene (Mystic)');
      console.log('  - returnToMainScene(): Return to farm view');
      console.log('  - listAllScenes(): List all available scenes and their status');
    }
  }
  ...
  createPhaserTilemapFromLayout() {
    ...
    import('../lib/tilemap.config').then(({ TilemapUtils }) => {
      ...
      console.log('🗺️ Rich tilemap world generated with advanced terrain features');
    });
  }
  ...
  setupTilemapCollision(map: Phaser.Tilemaps.Tilemap, layer: Phaser.Tilemaps.TilemapLayer) {
    ...
    import('../lib/tilemap.config').then(({ TilemapUtils }) => {
      ...
      console.log('🧱 Tilemap collision system initialized');
    });
  }

  initializeCollisionMap() {
    // Enhanced collision detection will be handled by tilemap layer
    console.log('🎯 Enhanced collision system ready');
  }
  ...
  /**
   * Calculate crop growth stage based on server data
   */
  calculateCropStage(crop: any): 'seed' | 'sprout' | 'growing' | 'mature' | 'ready' {
    const now = Date.now();
    const plantedTime = new Date(crop.plantedAt).getTime();
    const timeElapsed = (now - plantedTime) / 1000; // Convert to seconds
    const growthTime = crop.growthTime || 60; // Default 60 seconds if not specified
    
    const growthProgress = Math.min(timeElapsed / growthTime, 1);
    
    if (growthProgress >= 1) return 'ready';
    if (growthProgress >= 0.8) return 'mature';
    if (growthProgress >= 0.5) return 'growing';
    if (growthProgress >= 0.2) return 'sprout';
    return 'seed';
  }
  ...
  async connectToServer() {
    try {
      ...
      console.log('🎮 Setting up simple message-based multiplayer');
      ...
      this.room.onStateChange.once((state) => {
        ...
        if (state.players) {
          state.players.forEach((player: any, sessionId: string) => {
            console.log('🎮 Adding existing player:', sessionId, player.name);
            this.addPlayer(sessionId, player);
          });
        }
        
        // Load crops from server state
        if (state.crops && this.cropSystem) {
          const cropsArray = Array.from(state.crops.values()).map((crop: any) => ({
            id: crop.id,
            type: crop.seedType,
            x: crop.x,
            y: crop.y,
            plantedAt: new Date(crop.plantedAt).getTime(),
            stage: this.calculateCropStage(crop),
            health: 100
          }));
          
          this.cropSystem.loadCropsFromServer(cropsArray);
          console.log('🌾 Loaded', cropsArray.length, 'crops from server');
        }

        // Subscribe to real-time crop state changes (after state is available)
        this.room.state.listen('crops', (crops) => {
          console.log('🌾 Crops state changed on server');
        });

        // Subscribe to individual crop additions
        this.room.state.crops?.onAdd((crop: any, cropId: string) => {
          console.log('🌱 Crop added:', cropId);
          if (this.cropSystem) {
            this.cropSystem.addCropFromServer({
              id: crop.id,
              type: crop.seedType,
              x: crop.x,
              y: crop.y,
        ...
        // Subscribe to individual crop removals
        this.room.state.crops?.onRemove((crop: any, cropId: string) => {
          console.log('🗑️ Crop removed:', cropId);
          if (this.cropSystem) {
            this.cropSystem.removeCropFromServer(cropId);
          }
        });
      });
  ...
  createTestPlayerForDebugging() {
    ...
    this.room = {
      send: (type: string, data: any) => {
        console.log(`🧪 Mock room send: ${type}`, data);
    ...
    console.log('🧪 Test player created with mock room - movement should work');
  }
  ...
  plantCrop(cropType: CropType, x: number, y: number): void {
    if (this.cropSystem) {
      // Use server synchronization instead of local planting
      this.cropSystem.plantCropWithServer(x, y, cropType);
    }
  }
  ...
  harvestCropAtPosition(x: number, y: number): void {
    ...
    const crop = this.cropSystem.getCropAtPosition(x, y);
    if (crop) {
      // Use server synchronization instead of local harvesting
      this.cropSystem.harvestCropWithServer(crop.id);
    }
  }
...
function Game({ worldId, isOwnWorld }: GameProps) {
  ...
  useEffect(() => {
    ...
    console.log('🎬 Available scenes:', gameRef.current.scene.scenes.map(s => s.scene.key));
    ...
    <div className="game-wrapper">
      ...
      >
        <BuildingContextMenu
          ...
          onInfo={() => {
            console.log('🏗️ Building info requested:', buildingContextMenu?.building);
