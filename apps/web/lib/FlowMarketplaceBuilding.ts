...
export class FlowMarketplaceBuilding extends Phaser.GameObjects.Container {
  ...
  private createPromptText() {
    this.promptText = this.scene.add.text(0, -60, "Press E for Flow DeFi", {
      fontSize: "16px",
      color: "#ffffff",
  ...
  private onPlayerEnterRange() {
    console.log("🟣 Player entered Flow marketplace interaction range");
  ...
  private onPlayerExitRange() {
    console.log("🟣 Player exited Flow marketplace interaction range");
  ...
  checkInteraction(): boolean {
    if (
      ...
      console.log("🟣 Flow Marketplace interaction triggered!");
      this.scene.events.emit("flowMarketplaceInteraction");
